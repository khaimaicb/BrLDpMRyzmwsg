namespace Endgame
{
	...
	public sealed class ImageList
	{
		...
		public sealed class ImageCollection : IList, ICollection, IEnumerable
		{
			private class Entry
			{
				public Entry(string key, Sprite sprite, int width = 0, int height = 0)
				{
					this.Key = key;
					this.Sprite = sprite;
					this.Width = width;
			...
			public void Add(string key, Sprite image, int width, int height)
			{
				Entry result = this.imageList.Find(entry => entry.Key == key);
				...
				{
					this.imageList.Add(new Entry(key, image, width, height));
				}
				this.Owner.RaiseOnChangedEvent();
			}
